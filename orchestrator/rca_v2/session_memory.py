"""
Session Memory Manager for RCA v2
Manages per-complaint session state with Redis persistence
"""
...
class SessionMemoryManager:
    """
    Manages session-based memory for RCA v2
    ...
    def _get_session_key(self, complaint_id: str) -> str:
        ...
        return f"rca_v2:session:{complaint_id}"
    ...
    def add_iteration(
        ...
        Args:
            ...
            causes_evaluated: All causes evaluated in this iteration
    ...
    def mark_complete(self, session: SessionState, final_root_cause_id: str):
        ...
        Args:
            session: Current session state
            final_root_cause_id: Final root cause ID
        """
    ...
    def is_cause_already_evaluated(self, session: SessionState, cause: Dict[str, Any]) -> bool:
        ...
        Uses exact text matching (case-insensitive) to detect duplicates.
        Only filters causes that are EXACTLY the same text.
        
        Args:
            session: Current session state
            cause: Cause to check
        ...
        # Check by ID (only if from same source - don't filter evidence vs FMEA by ID)
        # Skip ID check since IDs are regenerated each iteration
        
        # Check by EXACT text match only
        # This ensures we only filter truly identical causes, not similar ones
        if cause_text:
    ...
    def filter_unevaluated_causes(self, session: SessionState, causes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Filter out causes that were already evaluated in this session
        
        Args:
            session: Current session state
            causes: List of causes to filter
            
        Returns:
            List of unevaluated causes
        """
        unevaluated = []
    ...
    def get_session_summary(self, session: SessionState) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_causes_evaluated": len(session.all_evaluated_cause_ids),
            "is_complete": session.is_complete,
            "session_duration_seconds": time.time() - session.session_start_time,
            "final_root_cause_id": session.final_root_cause_id
        }
